...
| `allow-incomplete`      | `false`                             | Do not fail the job when a file could not be reviewed; `fail-on` alone decides. The summary still says incomplete, and the comment job still will not supersede.                                        |
