...
export const {{STEP_NAME}}Step = createStep(
  '{{STEP_NAME}}-step',
  async (input: {{STEP_NAME_PASCAL}}StepInput, { container }): Promise<StepResponse<{{STEP_NAME_PASCAL}}StepOutput, string>> => {
  ...
  async ({{COMPENSATION_PARAM}}: string, { container }) => {
    const {{SERVICE_VARIABLE}}: {{SERVICE_CLASS}} = container.resolve({{MODULE_CONSTANT}});

    await {{SERVICE_VARIABLE}}.{{COMPENSATION_METHOD}}({{COMPENSATION_PARAM}});
  }
);