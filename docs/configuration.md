...
| `settings.llm.base-url`                  | —                |    ✓    |  ✓   | Endpoint URL including the API prefix, e.g. `http://localhost:11434/v1`. Empty: the vendor's public endpoint -- for `local`, **the OpenAI API**. Every run names the model and the endpoint at INFO. |
| `settings.llm.api-key`                   | _required_       |    ✓    |  ✓   | The key, or `${VARIABLE}` to read it from the environment. See below.                                                                                                                                |
| `settings.language`                      | `en`             |    ✓    |  ✓   | Language of each finding's body: an ISO code (`tr`, `de`, `ja`, …) or a name (`Brazilian Portuguese`). A value that could not be a language's name -- a sentence, digits -- falls back to English.   |
