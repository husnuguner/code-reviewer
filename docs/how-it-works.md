...
| `bypassed`       | Every added line lies in a block a [`reviewer: by-pass` marker](#bypassing-a-block-from-the-code) took out of review; the model is not called. |
...
`bypassed` is the exception: it is decided per file after the file is read,
so `--preview`, which reads nothing, does not show it.
...
## Bypassing a block from the code

A comment in the code reading

```ts
// reviewer: by-pass - generated by protoc; regenerate, do not edit
export function decode(buffer: Uint8Array): Message {
  …
}
```

takes the block after it out of review. Any comment syntax works (`//`, `#`,
`/* */`, `<!-- -->`, a line of a doc comment); `bypass` and `by-pass` are both
...
- A marker on a line of its own names the next line of code, skipping
  comments and decorators (`@Component({…})`, `@app.route("/")`). A marker at
  the end of a line of code names that line's statement.
- From there, the block is the brace scope that opens on that statement --
  the same line, an Allman-style `{` on the next, or the last line of a
  wrapped signature -- through its matching `}`, and on through any `else`,
  `catch` or `finally` that follows. Braces inside strings and comments do
  not count.
- A statement ending in `:` (Python `def`, `if`, `try`, …) names its
  indentation block, and on through `elif`, `else`, `except`, `finally` at
  the same indent.
- Anything else is one statement, to where its parentheses balance (a wrapped
  call included). In markup, which has neither, that is the next line.

The region runs from the marker line to the block's end. Overlapping regions
