...
| exit 2, `settings.<key>: must be …: value was "…"`                 | A setting has the wrong shape; the message names it by its place in the file (or the variable it came from). Every problem is listed at once.                                                                                                                            |
...
| exit 2, `git … did not finish within 120s and was stopped`         | A git command hung: a lock (`.git/index.lock` left by a crashed git), a credential prompt, or a hook. Remove the stale lock or fix the hook; the reviewer stops rather than holding the run.                                                                             |
