...
} from "../../../src/providers/reporting/builtin";
import { type ReportContext } from "../../../src/providers/reporting/format-provider";
...
describe("the built-in report formats", () => {
  const registry = builtinFormatProviders();
  ...
  it("registers text, ndjson and github, with text first", () => {
    ...
    expect(BUILTIN_FORMAT_PROVIDERS.map((format) => format.name)).toEqual(registry.names());
  });
  ...
  it("names the formats that exist when asked for one that does not", () => {
    expect(() => registry.create("xml", sink().context)).toThrow(ValueError);
    expect(() => registry.create("xml", sink().context)).toThrow(/'text'/u);
  });

  it("builds a reporter for every registered name", () => {
    for (const name of registry.names()) {
      expect(() => registry.create(name, sink().context)).not.toThrow();
    }
  });
});
...
describe("what each format writes", () => {
  ...
  it("github annotates as it goes and writes the summary at the end", () => {
    ...
    const reporter = registry.create("github", context);
    run(reporter);
  ...
  it("github writes no summary outside a runner, and still annotates", () => {
    ...
    const lines: string[] = [];
    const reporter = registry.create("github", {
      write: (text) => {
        lines.push(text);
      },
      summary: null,
