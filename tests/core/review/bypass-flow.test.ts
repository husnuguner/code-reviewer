...
/**
 * The feature marks `legacy` bypassed and adds a line at the top of its body, so the hunk ends three
 * lines into a body that runs on for twelve; then adds a line at the very end, outside any block.
 */
...
/**
 * `main` holds `svc.ts`; `feature` rewrites it as above and adds `whole.ts`, `bad.ts` and `plain.ts`.
 * The checkout is left on `feature`.
 */
...
describe("an uncommitted marker", () => {
  it("is read from the working tree and honoured like a committed one", async () => {
    const root = repo();
    // Author keeps working: marks `fresh` bypassed and adds a line to it, without committing.
    writeFileSync(
      join(root, "svc.ts"),
      SVC_AFTER.replace(
        "export function fresh() {\n  return x;",
    ...
    expect(result.findings.filter((f) => f.path === "svc.ts")).toEqual([]);
    expect(result.skipped).toEqual({ bypassed: 1 });
    expect(result.bypass_regions).toEqual([
      { path: "svc.ts", start_line: 3, end_line: 7, reason: "wip" },
      {
        path: "svc.ts",
