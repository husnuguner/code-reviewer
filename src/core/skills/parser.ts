 ...
 *
 * Frontmatter must declare `name`; `applies_to` (globs) and `description` are
 * optional -- the project's catalogue may say which files a skill reviews
 * instead. Returns `null` (logging a warning) when frontmatter is missing, the
 * YAML is invalid, or `name` is absent. YAML is read with 1.1 semantics (`yes`
 * is a boolean, `1_000` a number), the dialect skill authors have been writing
 * against.
 */
export class FrontmatterSkillParser implements SkillParser {
  ...
  parse(text: string, source: string): Skill | null {
    ...
    return {
      name,
      globs: normalizeGlobs(meta["applies_to"]),
      body: pyStrip(body),
      source,
...
/** Normalise `applies_to` (string or list) to non-empty, trimmed globs. */
function normalizeGlobs(appliesTo: unknown): string[] {
  const list = typeof appliesTo === "string" ? [appliesTo] : appliesTo;
  return Array.isArray(list)
    ? list.map((glob) => pyStrip(pyString(glob))).filter((glob) => glob !== "")
    : [];
}
